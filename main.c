...
#include <locale.h>
#include <string.h>
//#include <windows.h>  // Для платформы windows
...
void heapify(long long *, int, int, long long *, long long *);  // Процедура преобразования в двоичную кучу поддерева с корневым узлом i, что является
...
{
    //SetConsoleOutputCP(CP_UTF8); // Для корректного отображения кириллицы на платформе windows
...
Параметры:\n\
    --len [N] или -L   Длина генерируемого массива (натуральное число, обязательный параметр)\n\
    --arrayType или -A [atype] Тип генерации массива (по умолчанию: u):\n\
        s - отсортированы в порядке невозрастания\n\
        r - отсортированы в порядке возрастания\n\
        u - несортированный\n\
    --sortType или -S [stype] Используемый алгоритм сортировки (по умолчанию: 2):\n\
...
main.exe --len 50 --arrayType r --sortType 1\n\
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if (!strcmp(argv[i], "--len") || !strcmp(argv[i], "-L"))
        {
            fl_len = 1;
            if (i + 1 > argc - 1 || sscanf(argv[i + 1], "%d", &N) != 1 || (N <= 0))
            {
                printf("Неправильный аргумент для параметра '%s'. Справочная информация: \n\t%s --help или -H\n", argv[i], argv[0]);
                return 0;
            }
            i++;
            continue;
        } 
        else if (!strcmp(argv[i], "--arrayType") || !strcmp(argv[i], "-A"))
        {
        ...
        else if (!strcmp(argv[i], "--sortType") || !strcmp(argv[i], "-S"))
        {
        ...
        {
            printf("Неизвестный параметр '%s'. Справочная информация: \n\t%s --help или -H\n", argv[i], argv[0]);
            return 0;
        }
    }
...
void selectionSort(long long *mas, int N)
{
    ...
    printf("Selection sort: перемещений − %lld, сравнений − %lld \n" , cmp , mov);
}
...
void heapSort(long long* arr, int n) {
    ...
    printf("Heapsort: перемещений − %lld, сравнений − %lld \n" , cmp , mov);
}
